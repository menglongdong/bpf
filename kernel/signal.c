...
bool get_signal(struct ksignal *ksig)
{
	...
	/*
	 * 在这里，检查当前进程的冻结标志位，如果进程被冻结的话，就进入冻结状态。
	 * 可以看出，信号处理流程也是一个冻结点。
	 */
	try_to_freeze();
...
relock:
	...
	/*
	 * 这里会检查是否有待发送给父进程的信号（SIGNAL_CLD_CONTINUED或者SIGNAL_CLD_STOPPED）
	 */
	...
	for (;;) {
		...
		/* 如果线程组已经退出，那么直接返回KILL信号，并清除当前的所有信号。 */
		if ((signal->flags & SIGNAL_GROUP_EXIT) ||
		...
		/*
		 * 从当前进程的pending中取出一个synchronous信号来处理。synchronous信号
		 * 指的是由异常产生的信号，需要被优先处理。
		 */
		type = PIDTYPE_PID;
		...
		if (!signr)
			/* 
			 * 没有synchronous信号，那么从tsk->pending或者tsk->signal->shared_pending
			 * 中获取一个信号，优先从前者获取。由此可见，会优先处理私有信号。
			 */
		...
		/*
		 * 到这里，说明信号的处理函数是内核默认的。对于需要被忽略的信号，
		 * 直接跳过。
		 */
	...
	fatal:
		...
		/* 
		 * 如果信号是会引发进程coredump的信号，那么进入coredump流程。
		 * 可以看出，coredump在信号处理的前面。
		 * 
		 * SIG_KERNEL_COREDUMP_MASK中定义了会引发coredump的信号。
		 */
