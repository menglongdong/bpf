...
int arch_bpf_get_regs_nr(const struct btf_func_model *m)
{
	...
	return nr_regs > 6 ? -EOPNOTSUPP : nr_regs;
}
...
static __always_used __no_stack_protector notrace __noendbr unsigned long *
bpf_global_caller_run(unsigned long *args, unsigned long ip)
{
	...
	/* save the origin function ip for bpf_get_func_ip(), align to 16 bytes */
	*(args - 2) = ip & 0xfffffffffffffff0;

	rcu_read_lock_dont_migrate();
	md = __kfunc_md_get(ip - MCOUNT_INSN_SIZE);
	*(args - 1) = md->nr_args;
