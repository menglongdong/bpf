...
static void test_attach_mix(bool fentry_over_multi)
{
	...
	if (fentry_over_multi) {
		...
		if (!ASSERT_OK(tracing_multi_mix__load(skel_multi), "load"))
			goto cleanup;
		if (!ASSERT_OK(tracing_multi_mix__attach(skel_multi), "attach"))
			goto cleanup;
		if (!ASSERT_OK(tracing_multi_mix__load(skel), "load"))
			goto cleanup;
		if (!ASSERT_OK(tracing_multi_mix__attach(skel), "attach"))
			goto cleanup;
	} else {
