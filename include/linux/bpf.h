...
/* 检查器所检查的指令数量的上限值。这里说的指令数量并不是BPF程序编译后的指令，也
 * 不是实际会运行到的指令数量，而是检查器所检查的指令数量。
 *
 * 对于BPF程序中存在jmp指令（或者loop）的情况，检查器会检查所有的代码分支的指令，
 * 而这里限制的就是所有的branches的涵盖的指令上限。因此对于逻辑稍微复杂的BPF程序
 * （存在不可控loop，即不能完全确定实际loop的次数），很容易就会发生指令达到上限
 * 的情况。
 *
