			    ...
			    struct bpf_attach_target_info *tgt_info)
{
	...
	case BPF_TRACE_FEXIT_MULTI:
		...
		if (prog->sleepable) {
			...
			switch (prog->type) {
			case BPF_PROG_TYPE_TRACING:
				/* For now, sleepable prog is not supported yet for
				 * tracing multi-link to make thing simpler.
				 */
	...
	if (prog->type == BPF_PROG_TYPE_LSM) {
		...
		bpf_log(log, "Attaching tracing programs to function '%s' is rejected.\n",
			tname);
		module_put(mod);
		return -EINVAL;
		...
		bpf_log(log, "Attaching fexit/fmod_ret to __noreturn function '%s' is rejected.\n",
			tname);
		module_put(mod);
		return -EINVAL;
	}
